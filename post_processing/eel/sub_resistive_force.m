...
for nt = 1:num
    for nl = 1:nl_totl
        ...
        Cd = 1;dx=0.01;
        Cd2 = 0.06;
%         fd(nt,nl)=-1/2*Cd*dx*sz2(nl)*sum(temp_v.^2)*(1-cos_theta(nt,nl)^2);          %resistive force
%         fd(nt,nl)=-1/2*Cd*dx*sz2(nl)*sum(temp_v.^2)*cos_theta(nt,nl)^2;
        fd1(nt,nl)=-1/2*Cd*dx*sz2(nl)*sum(temp_v.^2)*(1-cos_theta(nt,nl)^2); 
%         fd2(nt,nl)=-1/2*Cd2*dx*sz2(nl)*sum(temp_v.^2)*cos_theta(nt,nl)^2;
%         force_resist2 (nt,nl,:)=fd(nt,nl)*vy_unit;
%         force_resist2 (nt,nl,:)=-fd(nt,nl)*body_unit;
        force_resist2(nt,nl,:) = fd1(nt,nl)*vy_unit; % - fd2(nt,nl)*body_unit;
%         fd(nt,nl)=-1/2*Cd*dx*sz2(nl)*sum(temp_v.^2);          %resistive force
%         force_resist2 (nt,nl,:)=fd(nt,nl)*veloc_unit;        
    end
    
end

%save force_inert force_inert
% lateral force
